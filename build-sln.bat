...
if "%SONAR_QUBE_TOKEN%" == "" (
	@echo "%CMD_MSBUILD%" %SLN_FILE% /p:Platform=%platform% /p:Configuration=%configuration% /p:PlatformToolset=v141_xp /t:"Build" %EXTRA_CMD% %LOG_OPTION%
	      "%CMD_MSBUILD%" %SLN_FILE% /p:Platform=%platform% /p:Configuration=%configuration% /p:PlatformToolset=v141_xp /t:"Build" %EXTRA_CMD% %LOG_OPTION%
) else (
    @echo "%BUILDWRAPPER_EXE%" --out-dir %~dp0bw-output "%CMD_MSBUILD%"  %SLN_FILE% /p:Platform=%platform% /p:Configuration=%configuration% /p:PlatformToolset=v141_xp /t:"Rebuild" %LOG_OPTION%
          "%BUILDWRAPPER_EXE%" --out-dir %~dp0bw-output "%CMD_MSBUILD%"  %SLN_FILE% /p:Platform=%platform% /p:Configuration=%configuration% /p:PlatformToolset=v141_xp /t:"Rebuild" %LOG_OPTION%
)
if errorlevel 1 (
